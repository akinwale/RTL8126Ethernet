/* RTL8126Setup.cpp -- RTL8126 data structure initialzation methods.
 *
...
void RTL8126::getParams()
{
    ...
    if (versionString)
        IOLog("RTL8126Ethernet version %s starting. Please don't support tonymacx86.com!\n", versionString->getCStringNoCopy());
    else
        IOLog("RTL8126Ethernet starting. Please don't support tonymacx86.com!\n");
}
...
bool RTL8126::initEventSources(IOService *provider)
{
    IOReturn intrResult;
    int msiIndex = -1;
    int intrIndex = 0;
    ...
    if (msiIndex != -1) {
        ...
        interruptSource = IOInterruptEventSource::interruptEventSource(this, OSMemberFunctionCast(IOInterruptEventSource::Action, this, &RTL8126::interruptHandler), provider, msiIndex);
    }
    if (!interruptSource) {
    ...
    timerSource = IOTimerEventSource::timerEventSource(this, OSMemberFunctionCast(IOTimerEventSource::Action, this, &RTL8126::timerActionRTL8126));
...
void RTL8126::refillSpareBuffers()
{
    mbuf_t m;
...
IOReturn RTL8126::refillAction(OSObject *owner, void *arg1, void *arg2, void *arg3, void *arg4)
{
...
bool RTL8126::setupStatResources()
{
    IODMACommand::Segment64 seg;
    UInt64 offset = 0;
    UInt32 numSegs = 1;
...
void RTL8126::freeStatResources()
{
    if (statBufDesc) {
        statBufDesc->complete();
        statBufDesc->release();
